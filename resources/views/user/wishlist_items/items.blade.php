...
<div class="col-sm-6 col-lg-4" style="position: absolute; left: 0px; top: 0px;">
    <div class="product mb-0">
        ...
        <div class="d-flex justify-content-between">
            ...
            <div class="addtocart-btn-wrapper">
                <form action="{{ route('user.wishlistItems.store') }}" method="POST" class="d-inline">
                    @csrf
                    <input type="hidden" name="inventory_id" value="{{ $inventory->id }}">
                    <input type="hidden" name="wishlist_id" value="{{ request('wishlist_id', $wishlist->id ?? null) }}">

                    @php
                    $alreadyAdded = $wishlist && $wishlist->wishlistItems->contains('inventory_id', $inventory->id);
                    @endphp

                    <button type="submit"
                        class="btn btn-link text-decoration-none text-color-default text-color-hover-dark text-4 p-0"
                        title="{{ $alreadyAdded ? 'Already in Wishlist' : 'Add to Wishlist' }}"
                        {{ $alreadyAdded ? 'disabled' : '' }}>
                        <i class="fa fa-gift {{ $alreadyAdded ? 'text-primary' : '' }}"></i>
                    </button>
                </form>
            </div>
        </div>
